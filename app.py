...
st.markdown("""
<style>
    /* Sleek Expand Buttons */
    .stButton > button { border: 1px solid #ccc; background-color: transparent; transition: 0.3s;}
...
def render_chart(num_name, den_name, period_str, interval_str, c_type, overlays, oscillators, height=650):
    ...
    # Enable TradingView Style Crosshairs
    fig.update_xaxes(
...
def expand_chart_modal(num_name, den_name):
    ...
    with st.spinner("Loading High-Res Interactive Engine..."):
        # The expanded view defaults to 1 Year, Candles, 50 SMA, 200 EMA, and RSI
        ...
        with cols_top[i]:
            ...
            with st.expander("📊 Quick Chart"):
                ...
                if top_fig: 
                    ...
                    # Pass TV_CONFIG to make quick charts interactive too
            ...
            with st.spinner("Fetching NSE Data..."):
                for idx, sec in enumerate(nse_list):
                    with cols[idx % 3], st.container(border=True):
                        # Create an inline expand button
                        head1, head2 = st.columns([4, 1])
                        head1.markdown(f"**{sec}**")
                        if head2.button("🗖", key=f"btn_nse_{idx}", help="Open Full Chart"): 
                            expand_chart_modal(sec, "Broad Market 500 (IND)")
                            
                        fig = render_chart(sec, "Broad Market 500 (IND)", "6mo", "1d", "Candlestick", [], [], height=250)
                        if fig: st.plotly_chart(fig, use_container_width=True, key=f"nse_{idx}", config={'displayModeBar': False})
            ...
            with st.spinner("Fetching US Data..."):
                for idx, sec in enumerate(us_list):
                    with cols[idx % 3], st.container(border=True):
                        head1, head2 = st.columns([4, 1])
                        head1.markdown(f"**{sec}**")
                        if head2.button("🗖", key=f"btn_us_{idx}", help="Open Full Chart"): 
                            expand_chart_modal(sec, "S&P 500 (US)")
                            
                        fig = render_chart(sec, "S&P 500 (US)", "6mo", "1d", "Candlestick", [], [], height=250)
                        if fig: st.plotly_chart(fig, use_container_width=True, key=f"us_{idx}", config={'displayModeBar': False})
            ...
            with st.spinner("Fetching Macro Data..."):
                for idx, (num, den, title) in enumerate(macro_pairs):
                    with cols[idx % 2], st.container(border=True):
                        ...
                        if head2.button("🗖", key=f"btn_glb_{idx}", help="Open Full Chart"): 
                            expand_chart_modal(num, den)
                            
                        fig = render_chart(num, den, "6mo", "1d", "Line", [], [], height=280)
                        if fig: st.plotly_chart(fig, use_container_width=True, key=f"glb_{idx}", config={'displayModeBar': False})
...
with tab2:
    ...
    with col_dyn_main:
        ...
        with st.spinner("Rendering Chart..."), st.container(border=True):
            ...
            if fig:
                # Pass the universal TV configuration with all drawing tools
                st.plotly_chart(fig, use_container_width=True, config=TV_CONFIG)
            elif selected_asset_name == "None" and benchmark_name == "None":
