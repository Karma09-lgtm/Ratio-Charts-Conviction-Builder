...
# This dictionary unlocks all advanced Plotly drawing tools for the TV experience
TV_CONFIG = {
    'modeBarButtonsToAdd': [
...
def render_chart(num_name, den_name, period_str, interval_str, c_type, overlays, oscillators, height=650):
    ...
    fig.update_xaxes(
        showgrid=True, gridcolor=TV_GRID, tickformat="%d %b '%y",
...
def expand_chart_modal(num_name, den_name):
    ...
    with st.spinner("Loading High-Res Chart..."):
...
with col_main:
    ...
    # Main Dynamic Chart (Fully Equipped)
    ...
    # The Static Macro Grid
    st.markdown("### 🌐 Global Macro Grid")
        ...
        with st.spinner("Fetching NSE..."):
            for idx, sec in enumerate(nse_list):
                with cols[idx % 3], st.container(border=True):
                    ...
                    # Expand Button triggers the Modal
                    if head2.button("🗖", key=f"btn_nse_{idx}"): expand_chart_modal(sec, "Broad Market 500 (IND)")
                    
                    fig_static = render_chart(sec, "Broad Market 500 (IND)", "3mo", "1d", "Line", [], [], height=200)
                    if fig_static: st.plotly_chart(fig_static, use_container_width=True, config={'displayModeBar': False})
                    
    with macro_tabs[1]:
        st.caption("Benchmark: S&P 500 (US)")
        us_list = ["Nasdaq 100", "Russell 2000", "US Tech ETF", "US Fin ETF", "US 20+ Yr Treasury", "VIX"]
        cols = st.columns(3)
        with st.spinner("Fetching US..."):
            for idx, sec in enumerate(us_list):
                with cols[idx % 3], st.container(border=True):
                    ...
                    fig_static = render_chart(sec, "S&P 500", "3mo", "1d", "Line", [], [], height=200)
                    if fig_static: st.plotly_chart(fig_static, use_container_width=True, config={'displayModeBar': False})

    with macro_tabs[2]:
        st.caption("Absolute Asset Performance")
        glb_list = ["Gold (Spot)", "Bitcoin", "Crude Oil", "Emerging Markets"]
        cols = st.columns(2)
        with st.spinner("Fetching Global Data..."):
            for idx, sec in enumerate(glb_list):
                with cols[idx % 2], st.container(border=True):
                    head1, head2 = st.columns([4, 1])
                    head1.markdown(f"**{sec}**")
                    if head2.button("🗖", key=f"btn_glb_{idx}"): expand_chart_modal(sec, "None")
                    
                    fig_static = render_chart(sec, "None", "3mo", "1d", "Line", [], [], height=200)
                    if fig_static: st.plotly_chart(fig_static, use_container_width=True, config={'displayModeBar': False})
...
with col_panel:
    ...
    else: st.caption("Watchlist is empty or data unavailable.")
    ...
    with st.popover("➕ Add Symbol to Watchlist", use_container_width=True):
        ...
        if add_asset == "Custom":
            custom_name, custom_tkr = st.text_input("Name"), st.text_input("Yahoo Ticker")
...
# --- SYSTEM FOOTER ---
st.markdown("---")
st.caption(f"⚡ **System Latency:** {round(time.time() - start_time, 2)}s | 📡 **Data Providers:** YF API, WSJ, CNBC")
