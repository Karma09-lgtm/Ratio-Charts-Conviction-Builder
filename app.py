...
# --- LIGHT CUSTOM CSS FOR PROFESSIONAL VIBE ---
st.markdown("""
<style>
    /* Clean up the expand buttons on the static grid */
    .stButton > button { transition: 0.3s; }
    .stButton > button:hover { border-color: #2962FF; color: #2962FF; }
    /* Make dataframes look cleaner */
...
if 'watchlists' not in st.session_state:
    st.session_state.watchlists = {
        "⭐ Core Favorites": {"S&P 500 (US)": "^GSPC", "Nifty 50": "^NSEI", "Gold (Spot)": "GC=F", "Bitcoin (USD)": "BTC-USD"},
        "🔥 Tech Watch": {"Nasdaq 100": "^NDX", "US Technology (XLK)": "XLK", "Nifty IT": "^CNXIT"}
    }
if 'active_wl' not in st.session_state:
    st.session_state.active_wl = "⭐ Core Favorites"
...
# --- TRADINGVIEW INTERACTIVITY CONFIG ---
TV_CONFIG = {
...
def fetch_yahoo_data(ticker, period, interval):
    try:
        ...
        # Flatten MultiIndex columns before doing anything else
...
def fetch_watchlist_quotes(ticker_dict):
    """Bulk downloads data for the watchlist and calculates % change."""
    if not ticker_dict: return []
    tkrs = list(ticker_dict.values())
    try:
        ...
        if isinstance(data.columns, pd.MultiIndex):
            ...
            if 'Close' in data.columns:
                data = data[['Close']].rename(columns={'Close': tkrs[0]})
            else: return []
        ...
        for name, tk in ticker_dict.items():
            if tk in data.columns:
                ...
                if len(col) >= 2:
                    ...
                    res.append({"Asset": name, "Price": last, "Chg %": chg})
                elif len(col) == 1:
                    res.append({"Asset": name, "Price": col.iloc[-1], "Chg %": 0.0})
        return res
    except: return []
...
def render_chart(num_name, den_name, period_str, interval_str, c_type, overlays, oscillators, height=650):
    ...
    # Enable TradingView Style Crosshairs (Spike Lines)
    fig.update_xaxes(
        showgrid=True, gridcolor=TV_GRID, tickformat=x_format, dtick=d_tick,
        showspikes=True, spikecolor="#888888", spikesnap="cursor", spikemode="across", spikethickness=1, spikedash="dash"
    )
    fig.update_yaxes(
...
def expand_chart_modal(num_name, den_name):
    ...
    # We pass standard TV indicators to the expanded view automatically
...
def render_watchlist(key_prefix):
    ...
    with st.container(border=True):
        ...
        wl_data = fetch_watchlist_quotes(st.session_state.watchlists[active_wl])
        if wl_data:
        ...
        with cols_top[i]:
            ...
            with st.expander("📊 Chart"):
                ...
                if top_fig: 
                    ...
                    # Passed TV_CONFIG to top charts
            ...
            with st.spinner("Fetching NSE Data..."):
                for idx, sec in enumerate(nse_list):
                    with cols[idx % 3], st.container(border=True):
                        ...
                        # Ensure scroll zoom is enabled even on the small static charts
                        if fig: st.plotly_chart(fig, use_container_width=True, key=f"nse_{idx}", config=TV_CONFIG)
                ...
                for idx, sec in enumerate(us_list):
                    with cols[idx % 3], st.container(border=True):
                        ...
                        if head2.button("🗖", key=f"btn_us_{idx}", help="Open Full Interactive Chart"): 
                            expand_chart_modal(sec, "S&P 500 (US)")
                            
                        fig = render_chart(sec, "S&P 500 (US)", "6mo", "1d", "Candlestick", [], [], height=250)
                        if fig: st.plotly_chart(fig, use_container_width=True, key=f"us_{idx}", config=TV_CONFIG)
            ...
            with st.spinner("Fetching Macro Data..."):
                for idx, (num, den, title) in enumerate(macro_pairs):
                    with cols[idx % 2], st.container(border=True):
                        ...
                        if fig: st.plotly_chart(fig, use_container_width=True, key=f"glb_{idx}", config=TV_CONFIG)
...
with tab2:
    ...
    with col_dyn_main:
        ...
        with st.spinner("Rendering Chart..."), st.container(border=True):
            ...
            if fig:
                # Apply TV_CONFIG here to ensure the dynamic explorer has all drawing tools
                st.plotly_chart(fig, use_container_width=True, config=TV_CONFIG)
            elif selected_asset_name == "None" and benchmark_name == "None":
